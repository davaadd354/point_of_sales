        ...
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-4 sm:p-6 text-gray-900">
                    ...
                    <form action="{{route('saveEditPengadaanProduk')}}" method="POST" class="space-y-4 sm:space-y-6">
                        @csrf
                        <input type="number" name="id" value="{{$pengadaan_produk->id}}" hidden>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                                ...
                                <div class="relative">
                                    <select name="produk_id" id="produk_id" required style="width: 100%;">
                                        ...
                                        @foreach ($data_produk as $pr)
                                            <option value="{{$pr->id}}" data-image="{{$pr->url_gambar}}" data-name="{{$pr->nama_produk}}" {{$pr->id == $pengadaan_produk->produk_id ? 'selected' : ''}}>{{$pr->nama_produk}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                                ...
                                <div class="relative">
                                    <select name="supplier_id" id="supplier_id" required
                                        ...
                                        @foreach ($data_supplier as $spl)
                                            <option value="{{$spl->id}}" {{$spl->id == $pengadaan_produk->supplier_id ? 'selected' : ''}}>{{$spl->nama_toko}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="space-y-2">
                                ...
                                <input type="number" name="jumlah_pcs" id="jumlah_pcs" required value="{{$pengadaan_produk->jumlah_pcs}}"
                            ...
                            <div class="space-y-2">
                                ...
                                <input type="number" name="harga_modal_per_pcs" id="harga_modal_per_pcs" required value="{{$pengadaan_produk->harga_modal_per_pcs}}"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
